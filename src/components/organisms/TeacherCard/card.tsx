...
interface CardDataProps {
  ...
  // カード内データ
	...
	onChangeValue: (id: number, data: { teacher: UserTypes.Teacher, user_name: string }) => void
  // 削除ボタンを押した時のイベントハンドラ
				...
				<Grid container spacing={0} columns={12}>
						...
						<Select
							...
							onChange={e => {
								if (typeof e.target.value === "string") {
									cardData.teacher.type = UserTypes.ConvertToNumberStudentPosition(e.target.value)
								} else {
					...
					<Grid xs={3}>
						<TextField
							label="生徒名"
							variant='standard'
							value={(props.data.teacher.user_id > 1) ? props.data.user_name: "未定"}
							margin="normal"
							sx={{ m: 1, width: '18ch' }}
							InputLabelProps={{ shrink: true }}
							color="primary" focused
