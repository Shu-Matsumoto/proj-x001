...
export const AddLectureSchedule = async (
  ...
  data: UserTypes.LectureSchedule
}> => {
...
export const AddLectureWithOptionData = async (
  ...
  result: UserTypes.AppResult
  data: UserTypes.LectureWithOptionData
}> => {
  let result = new UserTypes.AppResult()
  await AddLecture(context, lecture).then((apiResult) => {
    result = apiResult.result
    // 講義情報登録後に紐づけ必要なlecture_idをその他情報へセット
    students.forEach((student) => {
      student.lecture_id = apiResult.data.id
    })
    teachers.forEach((teacher) => {
      teacher.lecture_id = apiResult.data.id
    })
    schedules.forEach((schedule) => {
      schedule.lecture_id = apiResult.data.id
    })
    materials.forEach((material) => {
      material.lecture_id = apiResult.data.id
    })
  })
...
export const GetLectures = async (
  ...
  result: UserTypes.AppResult
  data: UserTypes.LectureWithUser[]
}> => {
  const address = `${context.apiRootUrl.replace(
    /\/$/g,
    '',
